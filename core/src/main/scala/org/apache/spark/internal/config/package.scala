...
package object config {
  ...
  private[spark] val LOCAL_REPARTITION_SENDER_BUFFER_SIZE =
    ConfigBuilder("spark.localRepartition.sender.buffer.size")
      .doc("The size of the buffer used to store the data in memory at sender when performing a " +
        "local repartition. This is only used when the local repartition is performed " +
        "in a single task. Note that this config value largely affects the " +
        "performance of the local repartition")
      .version("4.1.0")
  ...
  private[spark] val LOCAL_REPARTITION_RECEIVER_BUFFER_SIZE =
    ConfigBuilder("spark.localRepartition.receiver.buffer.size")
      .doc("The size of the buffer used to store the data in memory at receiver when performing " +
        "a local repartition. This is only used when the local repartition is performed " +
        "in a single task. Note that this config value largely affects the " +
        "performance of the local repartition")
      .version("4.1.0")
      .intConf
  ...
  private[spark] val LOCAL_REPARTITION_SENDER_MAXNUM =
    ConfigBuilder("spark.localRepartition.sender.maxNum")
      .doc("The maximum number of senders per operator when performing a local repartition. " +
        "This is only used when the local repartition is performed in a single task. Note that " +
        "this config value largely affects the performance of the local repartition")
      .version("4.1.0")
      .intConf
      .createWithDefault(Int.MaxValue)
