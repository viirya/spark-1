...
object ExpressionCodegen {
  ...
  def prepareFunctionParams(
    ...
    inputVars.flatMap { inputVar =>
      ...
      // Only include the expression value if it is not a literal and it is a valid
      // java variable name.
      if (!isLiteral(ev) && isJavaVariableName(ev)) {
  ...
  private[this] val javaVarPattern = new Regex("^[a-zA-Z_$][a-zA-Z_$0-9]*$")

  /**
   * Returns if the value in generated code for a given `ExprCode` is a valid java variable name.
   */
  def isJavaVariableName(exprCode: ExprCode): Boolean = {
    javaVarPattern.findFirstIn(exprCode.value).isDefined
  }
