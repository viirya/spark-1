...
private[sql] object SchemaPruning {
  ...
  def pruneSerializer(
    ...
    val structTypes = collectStructType(serializer.dataType, ArrayBuffer.empty[StructType])
    val prunedStructTypes = collectStructType(prunedDataType, ArrayBuffer.empty[StructType])

    // We can prune nested fields from serializers with a conservative approach. The struct types
    // in original schema must be aligned with the struct types in pruned schema. That is how we
    // match the pruned struct types to expressions creating struct (i.e., `CreateNamedStruct`).
    // Otherwise, we can't know what the pruned data types are for those `CreateNamedStruct`.
    if (structTypes.length == prunedStructTypes.length) {
      val structExprs = serializer.collect { case s: CreateNamedStruct => s }
        .zip(prunedStructTypes).toMap

      serializer.transformDown {
        case s: CreateNamedStruct =>
          val prunedType = structExprs(s)
          ...
          CreateNamedStruct(prunedFields._1 ++ prunedFields._2)
      }.transformDown {
