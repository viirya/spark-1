...
import org.apache.spark.sql.execution.StopAfter
import org.apache.spark.sql.functions._
import org.apache.spark.sql.internal.SQLConf
import org.apache.spark.sql.test.SharedSQLContext

class StopAfterSuite extends QueryTest with SharedSQLContext {
  ...
  test("Filter on sorted data: StopAfter should not change filter results") {
    ...
    Seq("true", "false").map { codeGen =>
      withSQLConf(SQLConf.WHOLESTAGE_CODEGEN_ENABLED.key -> codeGen) {
        val df2 = df.filter("id <= 2").toDF()
        assert(df2.queryExecution.executedPlan.collect {
          case _: StopAfter => true
        }.nonEmpty)
        checkAnswer(
          df2,
          Row(0) :: Row(1) :: Row(2) :: Nil)

        val df3 = df.filter("id < 10").groupBy().sum().toDF()
  ...
  test("Only do StopAfter if predicates are on the prefix sort by attributes") {
    // Prefix sort by attributes: "int", "int3"
    // int int2 int3 int4
    //   1    2    3    4
    //   2    3    4    5
    //   3    4    5    6
    ...
    Seq("true", "false").map { codeGen =>
      withSQLConf(SQLConf.WHOLESTAGE_CODEGEN_ENABLED.key -> codeGen) {
        ...
        // filter on "int" and "int2": no StopAfter
        ...
        val df7 = df.filter("int <= 2 AND int3 < 4").toDF()
  ...
  test("No StopAfter if child plan is not sorted on attributes") {
  ...
  test("No StopAfter if predicate is not binary comparison between attribute and literal") {
    ...
    //       We can remove the following 2 tests.
  ...
  test("No StopAfter if data contains null") {
    ...
    // attribute "int2" is not nullable. Can apply StopAfter with predicate on it.
    assert(output(1).name.equalsIgnoreCase("int2"))
    assert(output(1).nullable == false)
    Seq("true", "false").map { codeGen =>
      withSQLConf(SQLConf.WHOLESTAGE_CODEGEN_ENABLED.key -> codeGen) {
        assert(df.filter("int2 < 3").queryExecution.executedPlan.collect {
          case _: StopAfter => true
        }.nonEmpty)
        checkAnswer(
          df.filter("int2 < 3"),
  ...
  test("No StopAfter if predicates are not the same direction") {
    ...
    Seq("true", "false").map { codeGen =>
      withSQLConf(SQLConf.WHOLESTAGE_CODEGEN_ENABLED.key -> codeGen) {
        // "int <= 2" && "3 > int2" can be normalized as "int <= 2" && "int2 < 3".
        val df2 = df.filter("(int <= 2) AND (3 > int2)")
        assert(df2.queryExecution.executedPlan.collect {
          case _: StopAfter => true
        }.nonEmpty)
        checkAnswer(
    ...
    // "int <= 2" && "3 < int2" can be normalized as "int <= 2" && "int2 > 3".
    val df3 = df.filter("(int <= 2) AND (3 < int2)")
    assert(df3.queryExecution.executedPlan.collect {
      case _: StopAfter => true
    }.isEmpty)
