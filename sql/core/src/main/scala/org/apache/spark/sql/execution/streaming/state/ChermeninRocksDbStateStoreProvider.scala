 ...
 */
class ChermeninRocksDbStateStoreProvider extends StateStoreProvider with Logging {
  import ChermeninRocksDbStateStoreProvider._
  ...
  private val options: Options = new Options()
    ...
    .setWriteBufferSize(DEFAULT_WRITE_BUFFER_SIZE_MB * SizeUnit.MB)
    .setMaxWriteBufferNumber(DEFAULT_WRITE_BUFFER_NUMBER)
    .setMaxBackgroundCompactions(DEFAULT_BACKGROUND_COMPACTIONS)
  ...
  class RocksDbStateStore(val version: Long,
    ...
    override def id: StateStoreId = ChermeninRocksDbStateStoreProvider.this.stateStoreId
     ...
     *
     * @param start UnsafeRow having the `keyIndexOrdinal` column set with appropriate
     *              starting value.
    ...
    override def metrics: StateStoreMetrics =
      StateStoreMetrics(keysNumber, keysNumber * (keySchema.defaultSize + valueSchema.defaultSize),
        Map.empty)
    ...
    override def toString: String =
      s"RocksDbStateStore[id=(op=${id.operatorId},part=${id.partitionId})," +
        s"localDir=$dbPath,snapshotsDir=$baseDir]"
  ...
  private val localSnapshots: ConcurrentHashMap[Long, String] =
  ...
  override def getStore(version: Long): StateStore = synchronized {
    ...
    logInfo(s"Retrieved $stateStore for version $version of " +
      s"${ChermeninRocksDbStateStoreProvider.this} for update")
    stateStore
  }
  ...
  private def snapshot(version: Long, dbPath: String): Path = synchronized {
    ...
    try {
      if (removeCrcAndLogs(dbPath)) {
        ...
        throw new IOException(s"Failed to delete CRC files or old logs before moving $dbPath to " +
          s"$snapshotFile")
      }
    } catch {
      case ex: Exception =>
  ...
  private def loadDb(version: Long): String = {
    ...
    if (hasLocalSnapshot(version) && loadLocalSnapshot(version, dbPath) ||
    ...
    try {
      ...
      if (versions.nonEmpty) {
        ...
        if (filesToDelete.nonEmpty) {
          ...
          logInfo(s"Deleted files older than $earliestVersionToRetain for $this: " +
            s"${filesToDelete.mkString(", ")}")
        }
      }
    } catch {
      case NonFatal(e) =>
 ...
 */
object ChermeninRocksDbStateStoreProvider {
