...
private[parquet] class ParquetSchemaConverter(
  ...
  def convert(parquetSchema: MessageType): StructType = {
    println(s"messageType: $parquetSchema")
    parquetSchema.getPaths().asScala.zipWithIndex.map { case (paths, idx) =>
      println(s"path $idx: ${paths.mkString(", ")}")
    }
  ...
  def convertField(
    ...
    case t: GroupType =>
      println(s"field name in convertField: ${t.getName()}")
      val curPath = path ++ Seq(t.getName())
      println(s"curPath in convertField: $curPath")
